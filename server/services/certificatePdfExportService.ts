...
Generated: ${new Date(certification.generatedAt).toISOString()}
  ...
  private buildCertificateInfo(certification: any): string {
    ...
    lines.push(`Certificate ID: ${certification.certificateId}`);
    lines.push(`Version: ${certification.version}`);
    lines.push(`Generated At: ${new Date(certification.generatedAt).toISOString()}`);
    lines.push(`Source Snapshot: ${certification.sourceSnapshotId}`);

    lines.push('\n--- SIGNER INFORMATION ---\n');
    lines.push(`Name: ${certification.signer.userName}`);
    lines.push(`Email: ${certification.signer.userEmail}`);
    lines.push(`Role: ${certification.signer.userRole}`);
    lines.push(`Signed At: ${new Date(certification.signer.signedAt).toISOString()}`);
  ...
  private buildSignatureSection(certification: any): string {
    ...
    if (certification.digitalSignature) {
      ...
      lines.push(`Signed At: ${new Date(sig.signedAt).toISOString()}`);
      lines.push(`Signature: ${sig.signature.substring(0, 64)}...`);
  ...
  private buildTimestampSection(certification: any): string {
    ...
    if (certification.rfc3161Timestamp) {
      ...
      lines.push(`Timestamp: ${ts.timestamp}`);
      lines.push(`TSA Name: ${ts.tsaName}`);
      lines.push(`TSA URL: ${ts.tsaUrl}`);
  ...
  private buildAuditTrailSection(certification: any): string {
    ...
    if (certification.auditTrail) {
      ...
      lines.push(`Generated At: ${new Date(audit.generatedAt).toISOString()}`);
      lines.push(`Signature Algorithm: ${audit.signatureAlgorithm}`);
      lines.push(`Timestamp Authority: ${audit.timestampAuthority}`);
      lines.push(`Encryption Algorithm: ${audit.encryptionAlgorithm}`);
