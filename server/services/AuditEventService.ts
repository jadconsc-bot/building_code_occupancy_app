...
export class AuditEventService {
  ...
  async logAuditEvent(input: AuditEventInput): Promise<AuditEventResult> {
    ...
    try {
      ...
      // Extract insertId from Drizzle ResultSetHeader
      const eventId = Array.isArray(result) && result[0]?.insertId
        ? result[0].insertId
        : (result as any)?.insertId;

      if (!eventId) {
        throw new Error('Failed to extract insertId from insert result');
      }

      // Get the inserted record to retrieve server-generated timestamp
      const { eq } = await import('drizzle-orm');
      const insertedEvent = await (db as any)
        .select()
        .from(complianceAuditTrail)
        .where(eq(complianceAuditTrail.id, eventId))
        .limit(1)
        .then((rows: any[]) => rows[0]);

      if (!insertedEvent) {
        throw new Error(`Failed to retrieve inserted audit event with ID ${eventId}`);
      }
   ...
   * @param requestContext Request context (ip, userAgent, sessionId from actual request)
