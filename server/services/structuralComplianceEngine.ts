...
function evaluateStructuralRules(extraction: DrawingExtractionResult): RuleEvaluation[] {
  ...
  const studs = structural.memberSizes.filter((m: any) =>
  ...
  if (studs.length === 0) {
    ...
    const nonCompliant = studs.filter((s: any) => {
    ...
    results.push(nonCompliant.length > 0
      ? { ...RULE_STUD_MIN_SIZE, result: "FAIL",
          details: `Stud(s) appear smaller than minimum 38x89mm: ${nonCompliant.map((s: any) => `${s.label} (${s.dimension})`).join(", ")}. Verify with engineer.` }
  ...
  const joists = structural.memberSizes.filter((m: any) =>
  ...
  results.push(joists.length === 0
    ...
    : { ...RULE_JOIST_SIZE, result: "CONDITIONAL",
        details: `${joists.length} joist/rafter/beam member(s) identified: ${joists.map((j: any) => `${j.label} (${j.dimension})`).join(", ")}. Compliance with span tables requires professional verification.` }
  );
...
function evaluateConnectionRules(extraction: DrawingExtractionResult): RuleEvaluation[] {
  ...
  results.push(conn.fastenerTypes.length === 0
    ...
    : { ...RULE_FASTENERS, result: "CONDITIONAL",
        details: `${conn.fastenerTypes.length} fastener type(s) identified: ${conn.fastenerTypes.map((f: any) => f.type).join(", ")}. Compliance with NBC 9.23.9 fastener schedules requires professional verification.` }
  );

  const hasO86 = conn.csaStandards.some((s: any) => s.standard.includes("O86"));
  results.push({ ...RULE_CSA_O86,
    ...
    details: hasO86
      ...
      : conn.csaStandards.length > 0
        ? `CSA standards referenced: ${conn.csaStandards.map((s: any) => s.standard).join(", ")}. CSA O86 not explicitly referenced.`
