...
export async function detectRoomsFromPage(
  ...
  for (const region of regions) {
    ...
    const visionBuffer = visionScale < 1.0
      ? await sharp(croppedBuffer).resize(visionW, visionH, { fit: 'fill' }).jpeg({ quality: 90 }).toBuffer()
      : croppedBuffer;
  ...
  await saveRoomsToDb(filteredRooms, pageId, projectId, province, imgW, imgH, null);
  console.log('[RoomDetection] Saved', filteredRooms.length, 'rooms to DB for page', pageId);

  return {
    rooms: filteredRooms,
    metadata: {} as RoomDetectionResult['metadata'],
    pageNumber,
    modelVersion,
