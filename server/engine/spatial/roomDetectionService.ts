  ...
  await db.transaction(async (tx) => {
    ...
    for (const confirmed of confirmedRooms) {
      await tx.insert(detectedRooms).values({
        ...
        areaSqmJson: confirmed.areaSqmJson,
        floorLevel: confirmed.floorLevel,
        occupancyGroup: confirmed.occupancyGroup,
        occupancyGroupJson: confirmed.occupancyGroupJson,
        ...
        const polygonPromise = polygonQueue.add(async () => {
          ...
          await db2.update(detectedRooms)
            .set({
              ...
              ...(result.areaSqm !== null ? { areaSqmJson: { value: result.areaSqm, confirmed: false, source: 'derived' } } : {}),
