    ...
    calculatorResults: router({
      ...
      save: protectedProcedure
        ...
        .mutation(async ({ input, ctx }) => {
          ...
          // Writes a signed, immutable copy to calculationResults for legal
          // defensibility, alongside the existing projectCalculatorResults
          // write. Wrapped in try/catch so a failure here never blocks the
          // primary save. Applies to all calculator types as of Phase 2.
          //
          // Note: occupantLoad is saved by both OccupantLoadCalculator and
          // DrawingAnalysis. Both produce valid, independent audit records
          // here — this is fine for the audit trail (unlike
          // projectCalculatorResults/permitPackageRouter, where a pre-existing
          // .find() ambiguity is a SEPARATE, unrelated issue).
          try {
            ...
            await db.insert(calculationAuditLog).values({
              ...
              actor: ctx.user.id,
              timestamp: new Date(),
              details: `Audit trail dual-write: ${input.calculatorType}`,
            });
          } catch (auditError) {
            // Log but do not throw — primary save (projectCalculatorResults)
            // must succeed regardless of audit-trail write status.
            console.error('[AuditTrail] dual-write failed:', input.calculatorType, auditError);
          }
