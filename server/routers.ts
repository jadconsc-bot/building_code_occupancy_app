    ...
    calculatorResults: router({
      ...
      save: protectedProcedure
        ...
        .mutation(async ({ input, ctx }) => {
          ...
          // ── AUDIT TRAIL DUAL-WRITE (Phase 1 pilot: beamSpan only) ──────────
          // Writes a signed, immutable copy to calculationResults for legal
          // defensibility. Wrapped in try/catch so a failure here never blocks
          // the primary save — pilot validation only. Phase 2 will remove the
          // calculatorType gate once proven across more calculators.
          if (input.calculatorType === 'beamSpan') {
            try {
              ...
              await db.insert(calculationAuditLog).values({
                ...
                actor: ctx.user.id,
                timestamp: new Date(),
                details: 'Audit trail dual-write (Phase 1 pilot)',
              } as any);
            } catch (auditError) {
              // Log but do not throw — primary save (projectCalculatorResults)
              // must succeed regardless of audit-trail write status during pilot.
              console.error('[AuditTrail] Phase 1 dual-write failed for beamSpan:', auditError);
            }
          }
